...
export default function CertificateCard({ certificate }: { certificate: Certificate }) {
    return (
        <Link
            ...
            target="_blank"
            className="relative flex flex-1 flex-col py-6 px-5 group rounded-2xl border border-border bg-background transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1"
        >
            <div className="p-5 bg-primary text-background flex items-center justify-center w-14 h-14 rounded-full mb-5 transition-all duration-300 group-hover:bg-gradient-to-tr group-hover:from-primary group-hover:to-secondary group-hover:scale-110">
                <ArrowUpRight
                    size={22}
